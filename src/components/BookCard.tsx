...
import { Heart, BookOpen, Download, Clock, Tag, Star, Sparkles } from "lucide-react";
...
const BookCard = ({ title, image, color, readingTime, category, description, onClick, featured = false }: BookCardProps) => {
  const [isFavorited, setIsFavorited] = useState(false);

  return (
    <Card 
      className={cn(
        "group overflow-hidden border transition-all duration-300 hover:scale-[1.02] hover:-translate-y-1 relative",
        featured 
          ? "border-eco-green ring-2 ring-eco-green/50 shadow-lg shadow-eco-green/20 hover:shadow-2xl hover:shadow-eco-green/30 scale-[1.02]" 
    ...
    >
      ...
      {featured && (
        <>
          ...
          <div className="absolute top-0 left-0 right-0 z-20 bg-gradient-to-r from-eco-green via-eco-green to-eco-yellow py-2 px-4 flex items-center justify-center gap-2">
            ...
            <span className="text-white font-bold text-sm uppercase tracking-wide">Recommandé • Demo</span>
            <Star className="w-4 h-4 text-eco-yellow fill-eco-yellow" />
          </div>
        </>
      )}
      ...
      <div className={cn(
        ...
        {featured && (
          <div className="absolute -right-12 top-6 rotate-45 bg-eco-yellow text-eco-green font-bold text-xs py-1 px-12 shadow-md">
            ★ TOP CHOIX
          </div>
        )}
      ...
      <div className="p-5 space-y-3 relative z-10">
        <div className="flex items-start justify-between gap-2">
          <h3 className={cn(
            ...
            featured && "text-eco-green"
          )}>
            {title}
          </h3>
          {featured && (
            <Badge className="bg-eco-green text-white shrink-0 animate-bounce">
